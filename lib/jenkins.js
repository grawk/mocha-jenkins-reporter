...
function Jenkins(runner, options) {
  ...
  function genSuiteReport() {
    ...
    tests.forEach(function(test) {
      console.log('test', currentSuite.suite.fullTitle());
      var testCase = {
        'testcase': [{
          _attr: {
      ...
      if (options.screenshots) {
        ...
        if (options.screenshots == 'loop') {
          screenshot = path.join(screenshotDir, screenshots.find(function (element) {
            console.log(element, test.title, element.indexOf(test.title) >= 0);
            return element.indexOf(test.title) >= 0;
          }));
          screenshotIndex++;
        } else {
      ...
      if (test.state == "failed") {
        ...
        testCase.testcase.push({'system-err': ['[[ATTACHMENT|' + screenshot + ']]']})
